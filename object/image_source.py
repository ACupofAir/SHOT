...
def cal_acc_oda(loader, netF, netB, netC):
    ...
    all_output = nn.Softmax(dim=1)(all_output)
    _, predict = torch.max(all_output, 1)
    ent = torch.sum(-all_output * torch.log(all_output + args.epsilon), dim=1) / np.log(
        args.class_num
    )
    ent = ent.float().cpu()
    initc = np.array([[0], [1]])
    kmeans = KMeans(n_clusters=2, random_state=0, init=initc, n_init=1).fit(
        ent.reshape(-1, 1)
    )
            ...
            if iter_num % interval_iter == 0 or iter_num == max_iter:
                ...
                if args.dset == "VISDA-C":
                    acc_s_te, acc_list = cal_acc(
                        dset_loaders["source_te"], netF, netB, netC, True
                    )
                    log_str = (
                    ...
                    acc_s_te, _ = cal_acc(
                        dset_loaders["source_te"], netF, netB, netC, False
                    )
...
def test_target(args):
    ...
    if args.da == "oda":
        acc_os1, acc_os2, acc_unknown = cal_acc_oda(
            dset_loaders["test"], netF, netB, netC
        )
        log_str = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=2, help="batch_size (must be greater than 1)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="office-home",
        choices=["VISDA-C", "VISDA-M", "office", "office-home", "office-caltech"],
    )
